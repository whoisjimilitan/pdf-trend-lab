...
async function execute() {
  console.log("\n=== EXECUTING WITH VERIFIED DOMAIN ===\n");
...
|---|------|-------|-----------|--------|
${results.map((r, idx) => `| ${idx + 1} | ${r.business_name} | ${r.email} | ${r.message_id ? r.message_id.substring(0, 20) + '...' : 'N/A'} | ${r.status === 'SENT' ? '✅ SENT' : '❌ FAILED'} |`).join('\n')}
...
**Message ID**: ${r.message_id || 'Failed to send'}
...
**First reply = autonomous pipeline proven**
`;
